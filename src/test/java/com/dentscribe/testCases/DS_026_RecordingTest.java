...
public class DS_026_RecordingTest extends AndroidBase {
	...
	public void recordingTest() {
		...
		try {
			...
			int[] date = calendarPage.getDateMonthYear(readData("testData", "recordingTestDate"));
			...
			calendarPage.runAudio("", 5000);

			// __________________________________Verify continue button after pause________________________________________
			Thread.sleep(5000);
			assertTrue(calendarPage.verifyStopPauseButton());
			ExtentManager.logInfoDetails("<b>Stop, Pause</b> button is displayed as expected");
			calendarPage.clickPauseStopButton("pause");
			assertTrue(calendarPage.verifyPatientButton(calendarPage.patientName, "Continue"));

			// _________________________Again start recording______________________
			click(By.xpath("//android.widget.TextView[@text='" + calendarPage.patientName + "']//parent::android.view.ViewGroup//following-sibling::android.view.ViewGroup//android.widget.TextView[@text='Continue']"), driver);
			calendarPage.clickVerifyStartRecording("click", "Continue");
			calendarPage.clickWhileUsingAppButton();
			calendarPage.allowAccessMedia();
			Thread.sleep(5000);
			calendarPage.closeRecordingPausePopup();
